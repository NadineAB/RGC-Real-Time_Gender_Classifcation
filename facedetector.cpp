...
namespace cognition
{
	FaceDetector::FaceDetector(const std::string& ffaceCascadePath,
		FrameCapture* fcaptureDevice,
		bool foptimalSubregionDetect,
		double froiScaleFactor,
		const std::string& name)
		:Detector(name, fcaptureDevice), 
		 froiScaleFactor(froiScaleFactor), 
		 foptimalSubregionDetect(foptimalSubregionDetect)
	{
		ffaceClassifier.load(ffaceCascadePath);
	}
	...
	{
		if(ffaceClassifier.empty()) return;
				
		//getAreas is threadsafe
		RectVector lastRects = getAreas();
		/*if(gui::WebcamWidget::flag)
		{
          cv::Mat Sex=frame;
		cognition ::fisherfaceclassifer cgender;
		FaceGender=cgender.classdetection(lastRects,Sex);
		qDebug() << "Each Face " << FaceGender;
		}*/
		//
		try{

			if(!(foptimalSubregionDetect && lastRects.size() > 0))
			{
	...
	void FaceDetector::detectFacesInROI(RectVector &lastRects, const cv::Mat &frame)
	{
		...
		for(RectVectorItr i = lastRects.begin(); i != lastRects.end(); ++i)
		{
			...
			int adjustFactor = std::max((static_cast<int>(i->width * froiScaleFactor) - i->width) / 2, 35);
	...
	{
		ffaceClassifier.detectMultiScale(frame, /*the frame, input frame for detection*/
			...
			cv::Size(35, 35) /*min rect check size, the minimum!*/
			);
	}
}