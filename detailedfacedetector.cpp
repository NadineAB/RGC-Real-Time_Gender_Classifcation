...
namespace cognition
{
		...
		roiScaleFactor, name), detectWhat(detectWhat)
	{
		...
		setAutoNotify(false);  // ture to make automatically
	}
	...
	bool DetailedFaceDetector::loadCascade(DetectParams which, const std::string& cascadePath)
	{
		switch(which)
		{
		case FACE:  return ffaceClassifier.load(cascadePath); 
	...
	{	
		...
		eyeClassifier.detectMultiScale(eyeRegion, /*the frame, input frame for detection*/
			...
			1.1,  /*the the scale factor opencv uses to increase the window each pass, default 1.1*/
