...
namespace cognition
{
	...
	public:
		 ...
		 * 
		 */
		FaceDetector(const std::string& ffaceCascadePath, 
			FrameCapture* fcaptureDevice = 0,
			bool foptimalSubregionDetect = false,
			double froiScaleFactor = 1.16,
		 ...
		 */
		bool empty() const { 
			return ffaceClassifier.empty(); 
		}
     cv::CascadeClassifier ffaceClassifier;
	protected:
		...
		bool loadClassifier(const std::string& faceCascadePath){
			return ffaceClassifier.load(faceCascadePath);
		}
	};
}
